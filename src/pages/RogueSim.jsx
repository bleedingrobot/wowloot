...
function RogueSimPage() {
  const location = useLocation();
  const navigate = useNavigate();
  const [copyMessage, setCopyMessage] = useState("");
  const simUrl = useMemo(() => getRogueSimUrl(), []);
  ...
  const payload = useMemo(() => {
    ...
    const stored = sessionStorage.getItem(ROGUE_SIM_PAYLOAD_KEY)
      || localStorage.getItem(ROGUE_SIM_PAYLOAD_KEY);
  ...
  useEffect(() => {
    if (location.state?.simJsonText) {
      sessionStorage.setItem(ROGUE_SIM_PAYLOAD_KEY, location.state.simJsonText);
      localStorage.setItem(ROGUE_SIM_PAYLOAD_KEY, location.state.simJsonText);
    }
  }, [location.state]);

  const onCopyPayload = async () => {
    if (!payload) {
      setCopyMessage("No Rogue payload found. Return to Characters and launch again.");
      return;
    }
    ...
    try {
      ...
      setCopyMessage("Copied Rogue JSON. In the sim: Import > JSON Import > paste > Import.");
    } catch {
  ...
  return (
    <section className="panel sim-page">
      <div className="panel-heading">
        <div>
          <h2>Integrated {WOW_CLASSIC_LABEL} Rogue Sim</h2>
        ...
        <div className="sim-tip-card">
          ...
          {isLocalBridgeMode ? (
            <>
              ...
              <p className="subtitle">Your current Rogue payload is auto-loaded into WoWSims on page load.</p>
            </>
          ) : (
            <>
      ...
      <div className="sim-embed-wrap">
        <iframe
          title={`${WOW_CLASSIC_LABEL} WoWSims Rogue`}
          src={simUrl}
